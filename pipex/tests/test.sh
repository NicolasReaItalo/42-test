...
pipex_output="out/output_pipex"
bash_return_value=""
pipex_return_value=""
errors_pipex="out/errors_pipex"
...
TEST()
{
	...
	echo "${BCyan}Executing pipex command\n"
		echo "Errors output:"
	echo "${Cyan}######################################"
	echo "${Red}"
	...
	echo ${Cyan}
	echo "######################################"
	echo "\n${BCyan}$pipex_output content :"
	echo "${Cyan}######################################"
	cat $pipex_output
	echo "${Cyan}######################################\n"
	echo "${Cyan}Exit code value: [$pipex_return_value]\n"
	echo ""
	echo "${BWhite}Results:\n"


	diff $bash_output $pipex_output 2> /dev/null
	diff_r=$?
	if [ $diff_r -eq 0 ]
	then
		echo "${BGreen}[OK]	$pipex_output and $bash_output are identical"
	else
		echo "${BRed}[KO]	$pipex_output and $bash_output are not identical"
		failed_test=$((failed_test + 1))
	fi

	if [ $pipex_return_value -eq $bash_return_value ]
	then
...
pipex_output="out/output_interdit"
